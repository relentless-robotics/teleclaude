...
const LOG_FILE = path.join(LOGS_DIR, `mcp-${new Date().toISOString().split('T')[0]}.log`);
  ...
  try {
    ...
    if (method === 'initialize') {
      ...
      if (name === 'send_to_telegram') {
        ...
        if (!message) {
          log('WARN', 'send_to_telegram called with empty message');
        }
...
process.on('exit', (code) => {
  log('INFO', `MCP Server exiting with code ${code}`);
});

process.on('uncaughtException', (e) => {
  log('ERROR', 'Uncaught exception in MCP server', { error: e.message, stack: e.stack });
});
