 ...
 * Supports: Claude CLI, Anthropic API, OpenAI, Kimi
 *
 * Default priority: Claude CLI → Anthropic → OpenAI → Kimi
 * Override via config/llm_config.json or forceProvider('anthropic')
 *
...
// Load API keys from .env file (no dotenv dependency — simple KEY=VALUE parser)
...
// Also load from config/api_keys.json (legacy format)
...
try {
  if (fs.existsSync(API_KEYS_FILE)) {
    ...
    for (const [envVar, value] of Object.entries(keys)) {
      if (!process.env[envVar] && value && !envVar.startsWith('_')) {
...
// ============================================================================
// Usage Tracking: tracks token spend per provider per day
// ============================================================================
const USAGE_FILE = path.join(__dirname, '..', 'data', 'llm_usage.json');
...
function trackUsage(provider, inputTokens, outputTokens) {
  const today = new Date().toISOString().slice(0, 10);
  const usage = _loadUsage();
  if (!usage[today]) usage[today] = {};
  if (!usage[today][provider]) usage[today][provider] = { calls: 0, inputTokens: 0, outputTokens: 0, estimatedCostUsd: 0 };
  const entry = usage[today][provider];
  const config = PROVIDERS[provider];
  entry.calls++;
  entry.inputTokens += inputTokens || 0;
  entry.outputTokens += outputTokens || 0;
  if (config) {
    entry.estimatedCostUsd += ((inputTokens || 0) / 1000 * config.costPer1kIn) + ((outputTokens || 0) / 1000 * config.costPer1kOut);
  }
  usage[today] = usage[today]; // ensure set
  _saveUsage(usage);
  return entry;
}

function getUsageSummary(date) {
  const day = date || new Date().toISOString().slice(0, 10);
  const usage = _loadUsage();
  const dayData = usage[day] || {};
  let totalCost = 0;
  const breakdown = {};
  for (const [provider, stats] of Object.entries(dayData)) {
    totalCost += stats.estimatedCostUsd || 0;
    breakdown[provider] = { ...stats, estimatedCostUsd: (stats.estimatedCostUsd || 0).toFixed(4) };
  }
  return { date: day, totalCostUsd: totalCost.toFixed(4), breakdown };
}

/**
 * Find best provider for a specific task type (uses taskRouting config)
 * @param {string} taskType - e.g. 'trading_analysis', 'memory_health', 'complex_reasoning'
 */
function findProviderForTask(taskType) {
  const config = getLlmConfig();
  const routing = config.taskRouting || {};
  const preferred = routing[taskType];
  if (preferred && getApiKey(preferred) && !isCircuitOpen(preferred)) {
    return preferred;
  }
  // Fall back to normal priority
  return findAvailableProvider();
}

/**
 * Force all LLM calls to use a specific provider.
 * Call with null to reset to config/default behavior.
 * @param {string|null} provider - 'claude_cli', 'anthropic', 'openai', 'kimi', or null
 */
...
function isClaudeCliAvailable() {
  ...
  try {
    ...
    const result = execSync('claude --version', { encoding: 'utf-8', timeout: 5000, stdio: ['pipe', 'pipe', 'pipe'], windowsHide: true });
...
function callClaudeCli(prompt, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const cmd = `powershell -WindowStyle Hidden -Command "Get-Content '${tmpFile}' | claude -p --model ${model} --output-format text --tools '' --no-session-persistence"`;
...
function getProviderPriority() {
  ...
  return config.providerPriority || ['claude_cli', 'anthropic', 'openai', 'kimi', 'groq'];
}
...
function callLLM(provider, messages, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (config.isAnthropic) {
      ...
      // OpenAI-compatible format (OpenAI, Kimi)
...
async function callLLMWithFallback(messages, options = {}) {
  ...
  if (providers.length === 0) {
    ...
    const keyStatus = Object.entries(PROVIDERS).map(([name, cfg]) => {
      if (cfg.isClaude_cli) return `${name}: ${isClaudeCliAvailable() ? 'available' : 'not found'}`;
      return `${name}: ${process.env[cfg.envKey] ? 'key set' : 'NO KEY'}`;
    }).join(', ');
    throw new Error(`No LLM provider available (${keyStatus}). Set ANTHROPIC_API_KEY in .env or config/api_keys.json`);
  }
  ...
  for (const provider of providers) {
    try {
      ...
      // Track usage
      if (result.usage) {
        trackUsage(provider, result.usage.input_tokens || result.usage.prompt_tokens || 0, result.usage.output_tokens || result.usage.completion_tokens || 0);
      }
      return result;
    } catch (e) {
      lastError = e;
...
async function reasonAboutTrades(dataPackage, preferredProvider = null) {
  const provider = preferredProvider || findProviderForTask('trading_analysis');
  if (!provider) {
    throw new Error('No LLM provider available. Install Claude CLI, or set ANTHROPIC_API_KEY, OPENAI_API_KEY, or KIMI_API_KEY');
  }
