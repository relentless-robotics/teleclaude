...
Local IP: 10.0.0.2 (only reachable via Jupiter LAN hop)
Tailscale: TBD (NeedsLogin as of 2026-02-25)

Connection methods:
  1. Two-hop: PC -> Jupiter (192.168.137.2) -> Saturn (10.0.0.2)  [PRIMARY]
...
# ── Server constants (from environment) ───────────────────────────────────────
...
def run_on_saturn_via_jupiter_ssh(command, timeout=120):
    ...
    try:
        # Jupiter runs: ssh saturn@10.0.0.2 '<command>'
        wrapped = (
