...
const JUPITER_HOST = process.env.JUPITER_HOST || '192.168.137.2';
const JUPITER_USER = process.env.JUPITER_USER || 'jupiter';
const JUPITER_PASS = process.env.JUPITER_PASS || '';
const SATURN_HOST = process.env.SATURN_HOST || '10.0.0.2';
const SATURN_USER = process.env.SATURN_USER || 'saturn';
const SATURN_PASS = process.env.SATURN_PASS || '';
...
try:
    ssh.connect('${host}', username=os.environ.get('JUPITER_USER', 'jupiter'), password=os.environ.get('JUPITER_PASS', ''), timeout=10)
...
try:
    jhost = os.environ.get('JUPITER_HOST', '192.168.137.2')
    shost = os.environ.get('SATURN_HOST', '10.0.0.2')
    jump.connect(jhost, username=os.environ.get('JUPITER_USER', 'jupiter'), password=os.environ.get('JUPITER_PASS', ''), timeout=10)
    transport = jump.get_transport()
    channel = transport.open_channel('direct-tcpip', (shost, 22), (jhost, 0))
    target = paramiko.SSHClient()
    target.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    target.connect(shost, username=os.environ.get('SATURN_USER', 'saturn'), password=os.environ.get('SATURN_PASS', ''), sock=channel, timeout=10)
...
"`;
  ...
  const result = sshExec('192.168.137.2', countCmd, 60);
