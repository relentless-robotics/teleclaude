...
def main():
    ...
    try:
        jump.connect(jupiter_host,
                     username=os.environ.get('JUPITER_USER', 'jupiter'),
                     password=os.environ.get('JUPITER_PASS', ''), timeout=10)
        transport = jump.get_transport()
        channel = transport.open_channel('direct-tcpip', (saturn_host, 22), (jupiter_host, 0))

        target = paramiko.SSHClient()
        target.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        target.connect(saturn_host,
                       username=os.environ.get('SATURN_USER', 'saturn'),
                       password=os.environ.get('SATURN_PASS', ''), sock=channel, timeout=10)
