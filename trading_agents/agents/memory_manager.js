...
class MemoryManager {
  ...
  async run() {
    ...
    try {
      ...
      // AUTO-REMEDIATE safe issues before reporting
      let remediationResults = [];
      try {
        remediationResults = await this.autoRemediate(report);
        report.remediations = remediationResults;
      } catch (e) {
        console.warn('[MemoryManager] Auto-remediation error:', e.message);
        report.remediations = [];
      }

      // Run LLM analysis only when there are actual issues to analyze
      // (saves ~38 LLM calls/day when memory health is fine)
      let llmRecommendations = null;
      const needsLLM = report.healthScore < 90 ||
        (report.staleness && report.staleness.staleCount > 0) ||
        (report.dedup && report.dedup.contradictionCount > 0);
      if (needsLLM) {
        try {
  ...
  async checkTasksAndJobs() {
    ...
    // Only flag data files as unpersisted if they show meaningful new RESULTS
    // (e.g. daily reports that haven't appeared in memory) — not routine agent state updates
    const RESULT_FILES = ['daily_reports/'];  // only daily reports warrant memory persistence
    for (const df of result.dataFiles) {
      const isResultFile = RESULT_FILES.some(prefix => df.file.startsWith(prefix));
      if (isResultFile && df.ageHours < 48 && df.sizeBytes > 500) {
  ...
  async generateHealthReport({ staleness, dedup, fileSizes, index, sessions, tasksAndJobs }) {
    ...
    if (tasksAndJobs) {
      ...
      // Flag agents that haven't run in >24 hours (only for critical always-on agents)
      const criticalAlwaysOnAgents = ['memoryManager', 'securitySentinel'];
      for (const agent of (tasksAndJobs.agents || [])) {
        const thresholdHours = criticalAlwaysOnAgents.includes(agent.name) ? 24 : 48;
        if (agent.hoursAgo !== null && agent.hoursAgo > thresholdHours && agent.status !== 'disabled') {
          recommendations.push({
            type: 'AGENT_STALE',
            severity: 'LOW',
  ...
  // --------------------------------------------------------------------------
  // 7. AUTO-REMEDIATION — Safe, reversible actions executed automatically
  // --------------------------------------------------------------------------

  /**
   * Execute safe auto-fixes based on health report findings.
   * SAFE ACTIONS ONLY — no deletions, no MEMORY.md content edits, no contradiction resolution.
   * Returns array of { action, target, result, detail }
   */
  async autoRemediate(report) {
    const actions = [];

    // 1. Archive old session logs (>30 days)
    if (report.sessions.archiveCandidateCount > 0) {
      for (const candidate of report.sessions.archiveCandidates) {
        try {
          const result = this._archiveSessionLog(candidate.file);
          if (result) actions.push({ action: 'ARCHIVE_SESSION', target: candidate.file, result: 'OK', detail: `Moved to sessions/archive/ (${candidate.ageDays} days old)` });
        } catch (e) {
          actions.push({ action: 'ARCHIVE_SESSION', target: candidate.file, result: 'FAILED', detail: e.message });
        }
      }
    }

    // 2. Auto-trim oversized files (move DEAD/COMPLETED entries to archive)
    if (report.fileSizes.oversizedCount > 0) {
      for (const oversized of report.fileSizes.files) {
        try {
          const trimmed = this._autoTrimFile(oversized.file, oversized.limit);
          if (trimmed > 0) actions.push({ action: 'TRIM_FILE', target: oversized.file, result: 'OK', detail: `Trimmed ${trimmed} lines (was ${oversized.lineCount}, limit ${oversized.limit})` });
        } catch (e) {
          actions.push({ action: 'TRIM_FILE', target: oversized.file, result: 'FAILED', detail: e.message });
        }
      }
    }

    // 3. Add orphan files to INDEX.md
    if (report.index.orphanCount > 0) {
      try {
        const added = this._addOrphansToIndex(report.index.orphanFiles);
        if (added > 0) actions.push({ action: 'UPDATE_INDEX', target: 'INDEX.md', result: 'OK', detail: `Added ${added} orphan file(s) to INDEX.md` });
      } catch (e) {
        actions.push({ action: 'UPDATE_INDEX', target: 'INDEX.md', result: 'FAILED', detail: e.message });
      }
    }

    // 4. Clean up completed/old proposals (>7 days)
    const oldProposals = this.state.pendingProposals.filter(p => {
      if (p.status === 'PENDING') return false;
      const age = Date.now() - new Date(p.createdAt).getTime();
      return age > 7 * 24 * 60 * 60 * 1000;
    });
    if (oldProposals.length > 0) {
      this.state.pendingProposals = this.state.pendingProposals.filter(p => !oldProposals.includes(p));
      saveState(this.state);
      actions.push({ action: 'CLEAN_PROPOSALS', target: 'state', result: 'OK', detail: `Removed ${oldProposals.length} resolved proposal(s) older than 7d` });
    }

    if (actions.length > 0) {
      console.log(`[MemoryManager] Auto-remediated ${actions.length} issue(s): ${actions.filter(a => a.result === 'OK').length} OK, ${actions.filter(a => a.result === 'FAILED').length} failed`);
    }
  ...
  /**
   * Move a session log to sessions/archive/
   */
  _archiveSessionLog(filename) {
    const srcPath = path.join(SESSIONS_DIR, filename);
  ...
  /**
   * Auto-trim an oversized memory file by removing clearly dead/archived content.
   * Strategy: remove sections marked DEAD, COMPLETED, or old SESSION HANDOFF entries.
   * Returns number of lines trimmed.
   */
  _autoTrimFile(filename, limit) {
    ...
    let inOldHandoff = false;
    let handoffCount = 0;

    for (let i = 0; i < lines.length; i++) {
      const line = lines[i];

      // Detect DEAD strategy blocks in scoreboard tables — remove entire row
      if (/\|\s*.*\s*\|\s*.*DEAD.*\s*\|/.test(line) && filename === 'MEMORY.md') {
        continue; // Skip DEAD rows in tables
      }

      // Detect "PREVIOUS SESSION" handoff blocks — keep only the 2 most recent
      if (/^\*\*PREVIOUS SESSION/.test(line)) {
        handoffCount++;
        if (handoffCount > 2) {
          inOldHandoff = true;
          continue;
      ...
      // End of a handoff block is next ** header or ## header
      if (inOldHandoff && (/^\*\*[A-Z]/.test(line) || /^##/.test(line))) {
        inOldHandoff = false;
        handoffCount = 0; // Reset for next section
      }
      if (inOldHandoff) continue;

      newLines.push(line);
    }

    // Only write if we actually trimmed something meaningful
    const trimmed = originalCount - newLines.length;
    if (trimmed > 0) {
      // Backup before writing
      const backupPath = filePath + '.bak';
      fs.writeFileSync(backupPath, content, 'utf8');
      fs.writeFileSync(filePath, newLines.join('\n'), 'utf8');
      console.log(`[MemoryManager] Trimmed ${filename}: ${originalCount} → ${newLines.length} lines (backup: ${filename}.bak)`);
    }
    return trimmed;
  }

  /**
   * Add orphan files to INDEX.md with auto-detected descriptions.
   */
  _addOrphansToIndex(orphanFiles) {
    const indexPath = path.join(this.memoryDir, 'INDEX.md');
    ...
    for (const orphan of orphanFiles) {
      // Read first line of the orphan file for a description
      const orphanPath = path.join(this.memoryDir, orphan.file);
      let desc = '(auto-detected, needs description)';
      try {
        const firstLines = fs.readFileSync(orphanPath, 'utf8').split('\n').slice(0, 3);
        const titleLine = firstLines.find(l => l.startsWith('#'));
        if (titleLine) desc = titleLine.replace(/^#+\s*/, '').trim();
      } catch (e) {}

      // Append to INDEX.md
      if (!content.includes(orphan.file)) {
        content += `\n| ${orphan.file} | ${desc} | auto-added ${new Date().toISOString().slice(0, 10)} |`;
        added++;
      }
    }
  ...
  /**
   * Use LLM to analyze the health report and produce natural language
...
Respond in this JSON format:
{
  ...
  async reportToDiscord(report) {
    ...
    if (report.remediations && report.remediations.length > 0) {
      ...
      const failed = report.remediations.filter(r => r.result === 'FAILED');
      lines.push('');
      lines.push(`**Auto-fixed:** ${ok.length} action(s)${failed.length > 0 ? `, ${failed.length} failed` : ''}`);
      for (const r of ok.slice(0, 5)) {
        lines.push(`- [${r.action}] ${r.target}: ${r.detail}`);
      }
    }
    ...
    // LLM usage stats
    try {
      const { getUsageSummary } = require('../../utils/llm_reasoning');
      const usage = getUsageSummary();
      if (usage && Object.keys(usage.breakdown).length > 0) {
        lines.push('');
        const providers = Object.entries(usage.breakdown).map(([p, s]) => `${p}: ${s.calls} calls, $${s.estimatedCostUsd}`).join(' | ');
        lines.push(`**LLM Usage Today:** $${usage.totalCostUsd} (${providers})`);
      }
    } catch (e) {}
