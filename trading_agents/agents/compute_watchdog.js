...
const LVL3QUANT_PATH = 'C:\\Users\\Footb\\Documents\\Github\\Lvl3Quant';
...
const MEM_WARNING_PCT = 80;    // >80% used = WARNING (lowered from 90%)
const MEM_CRITICAL_PCT = 85;   // >85% used = CRITICAL — KILL TRAINING PROCESSES

// ── HARDCORE Neptune Resource Limits ─────────────────────────────────────────
// These limits are NON-NEGOTIABLE. Exceeding them crashed the PC.
const NEPTUNE_MAX_PYTHON_PROCS = 16;     // Max Python processes on Neptune
const NEPTUNE_RAM_KILL_PCT = 85;         // Kill training processes above this
const NEPTUNE_GPU_ONLY = true;           // Training MUST use GPU, not CPU
...
class ComputeWatchdog {
  constructor() {
    ...
    this.machines = {
      ...
      jupiter: { name: 'Jupiter', type: 'ssh', ip: '192.168.137.2' },
      saturn: { name: 'Saturn', type: 'ssh-hop', ip: '10.0.0.2' },
    };
  ...
  _checkThresholds(machine, status) {
    ...
    // Memory — HARDCORE enforcement for Neptune
    if (machine === 'pc' && status.mem >= MEM_CRITICAL_PCT) {
      alerts.push({
        severity: SEVERITY.CRITICAL,
        machine,
        machineName: name,
        type: 'mem_critical',
        message: `NEPTUNE RAM CRITICAL: ${status.mem}%! Guardian should be killing processes. CHECK IMMEDIATELY.`,
      });
      // Trigger guardian kill directly
      this._neptuneEmergencyKill(status);
    } else if (status.mem > MEM_WARNING_PCT) {
      alerts.push({
        severity: SEVERITY.WARNING,
        machine,
        machineName: name,
    ...
    // NEPTUNE: Too many Python processes
    if (machine === 'pc') {
      const pyProcs = (status.activeJobs || []).filter(j =>
        j.type === 'python' || (j.name && j.name.toLowerCase().includes('python'))
      );
      if (pyProcs.length > NEPTUNE_MAX_PYTHON_PROCS) {
        alerts.push({
          severity: SEVERITY.CRITICAL,
          machine,
          machineName: name,
          type: 'worker_limit',
          message: `NEPTUNE WORKER LIMIT: ${pyProcs.length} Python processes (max ${NEPTUNE_MAX_PYTHON_PROCS})!`,
        });
      }

      // GPU-only check: heavy RAM + idle GPU = possible CPU training
      if (NEPTUNE_GPU_ONLY && status.mem > 60 && status.gpu < 5) {
        const heavyPy = (status.activeJobs || []).filter(j =>
          j.type === 'python' && j.gpuMemMB && parseInt(j.gpuMemMB) < 500
        );
        if (heavyPy.length > 0 || pyProcs.length > 3) {
          alerts.push({
            severity: SEVERITY.WARNING,
            machine,
            machineName: name,
            type: 'cpu_training',
            message: `NEPTUNE: High RAM (${status.mem}%) but GPU idle (${status.gpu}%). Training may be running on CPU — MUST use GPU!`,
          });
        }
      }
    }
  ...
  /**
   * Emergency kill — invoked when Neptune RAM hits critical.
   * Calls the guardian script to handle the kill.
   */
  _neptuneEmergencyKill(status) {
    try {
      const guardianPath = require('path').join(PROJECT_ROOT, 'utils', 'neptune_guardian.py');
      execSync(`python "${guardianPath}" --once`, {
        timeout: 30000,
        encoding: 'utf8',
        windowsHide: true,
        stdio: ['pipe', 'pipe', 'pipe'],
      });
      console.log('[ComputeWatchdog] Neptune Guardian emergency kill triggered');
    } catch (e) {
      console.error('[ComputeWatchdog] Guardian kill failed:', e.message);
    }
  }
