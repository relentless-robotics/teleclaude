...
// Import LLM fallback chain for rate-limit recovery
let llmReasoning = null;
try {
  llmReasoning = require('../utils/llm_reasoning');
} catch (e) {
  console.log('LLM reasoning fallback not available:', e.message);
}
...
const HEARTBEAT_FILE = process.env.TELECLAUDE_HEARTBEAT_FILE || path.join(BRIDGE_DIR, '.heartbeat');
const IS_WATCHDOG_MANAGED = process.env.TELECLAUDE_WATCHDOG === '1';
const IS_RESTART = process.env.TELECLAUDE_IS_RESTART === '1';
const RESTART_COUNT = parseInt(process.env.TELECLAUDE_RESTART_COUNT || '0');
...
function writeHeartbeatState() {
  try {
    const state = {
      ts: Date.now(),
      claudeAlive: claude !== null && !claude.killed,
      claudePid: claude?.pid || null,
      lastUserMsg: lastUserMessageTime || null,
      lastPtyOutput: lastPtyOutputTime,
      lastMcpResponse: lastClaudeResponseTime,
      pendingUserMsg: !userMsgAnswered,
    };
    fs.writeFileSync(HEARTBEAT_FILE, JSON.stringify(state));
  } catch (e) { /* ignore */ }
}

function startHeartbeat() {
  if (heartbeatInterval) clearInterval(heartbeatInterval);
  heartbeatInterval = setInterval(writeHeartbeatState, 15_000);
  writeHeartbeatState(); // Write immediately
}

// ── Usage Monitor ──────────────────────────────────────────────────────────
// Runs the usage scraper periodically and sends Discord alerts at thresholds
let usageMonitorInterval = null;
let lastUsageAlertLevel = null; // Track last alert to avoid spam: '75' | '90' | '100'

function startUsageMonitor() {
  // DISABLED — usage scraper reads local .jsonl files and uses hardcoded fake limits
  // that don't reflect actual Claude subscription usage (claude.ai/settings/usage).
  // This was producing false 197% alerts when real usage was 2%.
  // Re-enable only if scraper is rewritten to use actual API usage data.
  console.log('[UsageMonitor] DISABLED — scraper data is unreliable');
}

function checkUsageAndAlert() {
  try {
    const scraperPath = path.join(BRIDGE_DIR, 'utils', 'claude_usage_scraper.js');
    if (!fs.existsSync(scraperPath)) return;

    // Run scraper inline (it's synchronous)
    delete require.cache[require.resolve(scraperPath)];
    // The scraper writes to dashboard-app/data/claude_usage.json — read it after
    require(scraperPath);

    const usageFile = path.join(BRIDGE_DIR, 'dashboard-app', 'data', 'claude_usage.json');
    if (!fs.existsSync(usageFile)) return;

    const usage = JSON.parse(fs.readFileSync(usageFile, 'utf8'));

    // Check daily output tokens (most relevant for rate limiting)
    const dailyOutput = usage.daily?.output || 0;
    const dailyLimit = usage.limits?.daily?.outputLimit || 200_000;
    const dailyPct = dailyLimit > 0 ? (dailyOutput / dailyLimit * 100) : 0;

    // Check weekly
    const weeklyOutput = usage.weekly?.output || 0;
    const weeklyLimit = usage.limits?.weekly?.outputLimit || 1_000_000;
    const weeklyPct = weeklyLimit > 0 ? (weeklyOutput / weeklyLimit * 100) : 0;

    // Use the higher percentage
    const maxPct = Math.max(dailyPct, weeklyPct);
    const windowName = weeklyPct > dailyPct ? 'weekly' : 'daily';

    // Determine alert level
    let alertLevel = null;
    if (maxPct >= 100) alertLevel = '100';
    else if (maxPct >= 90) alertLevel = '90';
    else if (maxPct >= 75) alertLevel = '75';

    // Only alert if level changed (don't spam same level)
    if (alertLevel && alertLevel !== lastUsageAlertLevel) {
      lastUsageAlertLevel = alertLevel;

      const resetAt = windowName === 'weekly'
        ? usage.limits?.weekly?.resetAt
        : usage.limits?.daily?.resetAt;
      const resetTime = resetAt ? new Date(resetAt).toLocaleString('en-US', { timeZone: 'America/New_York' }) : 'unknown';

      let msg;
      if (alertLevel === '100') {
        msg = `**USAGE ALERT: LIMIT REACHED (${maxPct.toFixed(0)}%)**\n` +
          `${windowName} output tokens: ${dailyOutput.toLocaleString()} / ${dailyLimit.toLocaleString()}\n` +
          `Weekly: ${weeklyOutput.toLocaleString()} / ${weeklyLimit.toLocaleString()}\n` +
          `Resets: ${resetTime} ET\n` +
          `Use \`/models\` to switch providers if needed.`;
      } else if (alertLevel === '90') {
        msg = `**USAGE WARNING: ${maxPct.toFixed(0)}% of ${windowName} limit used**\n` +
          `Daily output: ${dailyOutput.toLocaleString()} / ${dailyLimit.toLocaleString()}\n` +
          `Weekly output: ${weeklyOutput.toLocaleString()} / ${weeklyLimit.toLocaleString()}\n` +
          `Resets: ${resetTime} ET\n` +
          `Consider reducing usage or switching tasks to Groq/Sonnet.`;
      } else {
        msg = `**Usage Notice:** ${maxPct.toFixed(0)}% of ${windowName} limit used\n` +
          `Daily: ${dailyOutput.toLocaleString()} / ${dailyLimit.toLocaleString()} | ` +
          `Weekly: ${weeklyOutput.toLocaleString()} / ${weeklyLimit.toLocaleString()}`;
      }

      if (currentChannel) {
        currentChannel.send(msg).catch(e => {
          console.error('[UsageMonitor] Failed to send alert:', e.message);
        });
      }
      console.log(`[UsageMonitor] Alert sent: ${alertLevel}% (${windowName})`);
    }

    // Reset alert level if usage drops below 75% (e.g., after reset)
    if (maxPct < 75 && lastUsageAlertLevel) {
      lastUsageAlertLevel = null;
    }
  } catch (e) {
    console.error('[UsageMonitor] Error:', e.message);
  }
}

// Auto-restart configuration for Claude PTY
const CLAUDE_RESTART_DELAY_MS = 3_000;
const CLAUDE_MAX_RESTART_ATTEMPTS = 10;
const CLAUDE_RESTART_BACKOFF_RESET_MS = 300_000; // Reset backoff after 5 min of stability
let claudeRestartAttempts = 0;
let claudeLastStableTime = null;

// Claude response tracking
let lastPtyOutputTime = Date.now();
let lastClaudeResponseTime = Date.now();      // Last time Claude sent a message via MCP
let userMsgAnswered = true;                   // Track whether the last user message got a response
// Auto-fallback variables — DISABLED. Groq/other models only used when
// user explicitly selects via /models command. Kept for /models integration.
let rateLimitFallbackInFlight = false;
let lastFallbackFailMsgTime = 0;
const FALLBACK_FAIL_MSG_COOLDOWN_MS = 60000;
...
// Persist last channel ID so messages aren't dropped on bridge restart
const LAST_CHANNEL_FILE = path.join(BRIDGE_DIR, '.last_channel_id');

function persistChannelId(channelId) {
  try { fs.writeFileSync(LAST_CHANNEL_FILE, channelId, 'utf8'); } catch (_) {}
}

/**
 * Get a usable channel for sending messages. Priority:
 * 1. currentChannel (DM channel from last user message)
 * 2. tradingChannels.general (always available after bot ready)
 * 3. Restored channel from persisted ID
 */
function getResponseChannel() {
  if (currentChannel) return currentChannel;
  if (tradingChannels.general) {
    console.log('[Fallback] Using #general since currentChannel is null');
    return tradingChannels.general;
  }
  // Try restoring from persisted channel ID
  if (client) {
    try {
      const savedId = fs.readFileSync(LAST_CHANNEL_FILE, 'utf8').trim();
      if (savedId) {
        const restored = client.channels.cache.get(savedId);
        if (restored) {
          console.log(`[Fallback] Restored channel from disk: ${savedId}`);
...
function startTradingScheduler() {
  ...
  try {
    ...
    // Create a send function that routes to dedicated trading channels
    const tradingSend = async (channelKey, message) => {
      // If called with a single string arg (legacy fallback from discord_channels.js),
      // check if the message contains a channelOnly prefix — if so, drop it silently.
      // This prevents channelOnly messages from ever reaching DM/general.
      if (message === undefined) {
        message = channelKey;
        channelKey = null;
      }
      if (channelKey && tradingChannels[channelKey]) {
        await sendToTradingChannel(channelKey, message);
      } else if (channelKey) {
        // channelKey specified but no tradingChannel — route through sendToTradingChannel
        // which has its own channelOnly guard
        await sendToTradingChannel(channelKey, message);
      } else {
        const ch = getResponseChannel();
        if (ch) {
    ...
    return new Promise((resolve, reject) => {
      ...
      https.get(url, (response) => {
        ...
        if (response.statusCode >= 300 && response.statusCode < 400 && response.headers.location) {
          ...
          https.get(response.headers.location, (redirectResponse) => {
            if (redirectResponse.statusCode !== 200) {
              logger.loggers.bridge.error('Attachment redirect failed', { status: redirectResponse.statusCode });
              reject(new Error(`HTTP ${redirectResponse.statusCode} on redirect`));
              return;
            }
        ...
        if (response.statusCode !== 200) {
          file.close();
          fs.unlink(localPath, () => {});
          logger.loggers.bridge.error('Attachment download failed', { status: response.statusCode });
...
function isClaudeRateLimitOutput(text) {
  const lower = text.toLowerCase();
  return lower.includes("you've hit your limit") ||
    lower.includes('rate-limit-options') ||
    (lower.includes('what do you want to do?') && lower.includes('stop and wait for limit to reset'));
}

async function sendProviderFallbackResponse(userMessage, reason = 'claude_rate_limited') {
  if (!currentChannel) return false;

  if (!llmReasoning) {
    const now = Date.now();
    if (now - lastFallbackFailMsgTime > FALLBACK_FAIL_MSG_COOLDOWN_MS) {
      lastFallbackFailMsgTime = now;
      await currentChannel.send(
        'Claude is currently unavailable due to usage limits, and no provider fallback is configured. ' +
        'Use `/restart` after the limit resets.'
      ).catch(() => {});
    }
    lastClaudeResponseTime = Date.now();
    userMsgAnswered = true;
    return false;
  }

  if (rateLimitFallbackInFlight) return true;
  rateLimitFallbackInFlight = true;

  try {
    const configuredPriority = (llmReasoning.getProviderPriority?.() || []).filter((provider) => provider !== 'claude_cli');
    const priority = configuredPriority.includes('groq')
      ? ['groq', ...configuredPriority.filter((provider) => provider !== 'groq')]
      : configuredPriority;
    ...
    if (priority.length === 0) {
      ...
      if (now - lastFallbackFailMsgTime > FALLBACK_FAIL_MSG_COOLDOWN_MS) {
        lastFallbackFailMsgTime = now;
        await currentChannel.send(
          'Claude is rate-limited, and no alternate providers are configured. ' +
          'Add an API key such as `GROQ_API_KEY` in `.env`, or use `/restart` after reset.'
        ).catch(() => {});
      }
      lastClaudeResponseTime = Date.now();
      userMsgAnswered = true;
    ...
    const systemPrompt =
      'You are TeleClaude fallback mode. Claude Code CLI is currently unavailable or rate-limited. ' +
      'Respond helpfully and concisely to the user. Be transparent that you are in fallback mode. ' +
      'Do not claim to have tool access, file editing, shell execution, or persistent project memory in this mode. ' +
      'If the user asks for an action that needs tools, explain the limitation briefly and suggest retrying when Claude recovers.';
    ...
    for (const provider of priority) {
      try {
        response = await llmReasoning.callLLM(provider, [
        ...
        lastError = e;
        logger.loggers.bridge.warn('Fallback provider failed', {
          provider,
          reason,
          error: e.message,
        });
    ...
    if (!response?.content) {
      // Rate-limit the "fallback failed" message itself to prevent spam
      const now = Date.now();
      if (now - lastFallbackFailMsgTime > FALLBACK_FAIL_MSG_COOLDOWN_MS) {
        lastFallbackFailMsgTime = now;
        const msg = lastError?.message
          ? `**Claude is rate-limited, and fallback providers also failed:** ${lastError.message}\nUse \`/restart\` when the limit resets.`
          : 'Claude is rate-limited, and no fallback provider produced a response.';
        await currentChannel.send(msg).catch(() => {});
      }
      lastClaudeResponseTime = Date.now();
      userMsgAnswered = true;
      return false;
    ...
    const prefix = `Claude is rate-limited right now, so this reply is coming from fallback mode via **${response.provider}**.\n\n`;
    await sendChunked(prefix + response.content);
    logger.loggers.bridge.info('Fallback response sent', {
    ...
    rateLimitFallbackInFlight = false;
  }
}
...
function startClaude() {
  ...
  // Reset activity timestamps on restart to prevent stale freeze detection
  lastPtyOutputTime = Date.now();
  lastClaudeResponseTime = Date.now();
  userMsgAnswered = true; // No pending user message on fresh start
  ...
  claude.onData((data) => {
    ...
    // Track PTY activity for freeze detection
    if (clean) lastPtyOutputTime = Date.now();
    ...
    // Rate limit detection — log only, do NOT auto-fallback to Groq.
    // Fallback should ONLY happen when user explicitly selects it via /models command.
    if (clean && isClaudeRateLimitOutput(clean)) {
      logger.loggers.system.warn('Claude CLI rate limit screen detected (logged only, no auto-fallback)');
    }
  });
  ...
  claude.onExit((e) => {
    ...
    // Reset backoff if Claude was stable for a while
    if (uptime > CLAUDE_RESTART_BACKOFF_RESET_MS) {
      claudeRestartAttempts = 0;
    }

    // AUTO-RESTART DISABLED — was causing constant crash loops.
    // Claude exits normally (context window full, etc.) and does NOT need auto-restart.
    // Use /restart command manually if needed.
    logger.loggers.system.info('Claude PTY exited. Auto-restart DISABLED. Use /restart to restart manually.', {
      previousUptime: `${Math.round(uptime / 1000)}s`
    });
    if (currentChannel) {
      currentChannel.send('Claude session ended. Use `/restart` to start a new session.').catch(() => {});
    }
  });

  // Build init message based on whether this is a restart/recovery
  const buildInitMessage = () => {
    if (claudeRestartAttempts > 1 || IS_RESTART) {
      // Recovery mode: tell Claude to read chat history and context
      return `You are recovering from a crash/restart. This is restart #${claudeRestartAttempts || RESTART_COUNT}. ` +
        `Read the last 10 messages from your Discord DM channel using read_channel to understand what you were doing before the crash. ` +
        `Then check your pending memories with check_pending() and resume where you left off. ` +
        `Send a brief recovery status to Discord via send_to_discord.`;
    }
    return 'Confirm you\'re ready by using send_to_discord.';
  };
    ...
    setTimeout(() => {
      if (claude) {
        ...
        setTimeout(() => {
          ...
          logger.loggers.claude.info('Sending init message to Claude', { isRecovery: claudeRestartAttempts > 1 || IS_RESTART });
          console.log('Sending init...');
          claude.write(initMsg);
          setTimeout(() => {
            if (claude) {
...
function sendToClaude(userMessage) {
  ...
  userMsgAnswered = false;

  // No auto-fallback — always send to Claude. User must explicitly select
  // alternate models via /models command if they want Groq/other providers.
...
/restart - Restart Claude PTY only
/reset - Kill all Claude + restart fresh
/fullrestart - **Exit entire bridge** (watchdog auto-restarts)
/nuke - **Kill everything** (manual restart needed)
/kill - Kill ALL Claude processes
/ping - Check if bridge is responsive
/usage - Show Claude token usage and limits
...
**Resilience:**
- Watchdog restarts bridge on crash (\`npm run watchdog\`)
...
/help - Show this message`
  ...
  if (text === '/usage') {
    logger.logSystemCommand('/usage');
    try {
      const usageFile = path.join(BRIDGE_DIR, 'dashboard-app', 'data', 'claude_usage.json');
      // Run scraper first for fresh data
      try {
        delete require.cache[require.resolve(path.join(BRIDGE_DIR, 'utils', 'claude_usage_scraper.js'))];
        require(path.join(BRIDGE_DIR, 'utils', 'claude_usage_scraper.js'));
      } catch (e) {}

      if (!fs.existsSync(usageFile)) {
        await message.channel.send('Usage data not available. Run `node utils/claude_usage_scraper.js` first.');
        return true;
      }
      const usage = JSON.parse(fs.readFileSync(usageFile, 'utf8'));
      const dOut = usage.daily?.output || 0;
      const dLim = usage.limits?.daily?.outputLimit || 200000;
      const wOut = usage.weekly?.output || 0;
      const wLim = usage.limits?.weekly?.outputLimit || 1000000;
      const dPct = (dOut / dLim * 100).toFixed(1);
      const wPct = (wOut / wLim * 100).toFixed(1);
      const dReset = usage.limits?.daily?.resetAt ? new Date(usage.limits.daily.resetAt).toLocaleString('en-US', { timeZone: 'America/New_York' }) : '?';
      const wReset = usage.limits?.weekly?.resetAt ? new Date(usage.limits.weekly.resetAt).toLocaleString('en-US', { timeZone: 'America/New_York' }) : '?';
      ...
      await message.channel.send(
`**Claude Usage**
Daily output:  \`${bar(dPct)}\` **${dPct}%** (${dOut.toLocaleString()} / ${dLim.toLocaleString()})
Weekly output: \`${bar(wPct)}\` **${wPct}%** (${wOut.toLocaleString()} / ${wLim.toLocaleString()})
Daily msgs: ${usage.daily?.messages || 0} | Weekly msgs: ${usage.weekly?.messages || 0}
Resets: Daily ${dReset} ET | Weekly ${wReset} ET
Last scan: ${usage.generatedAt || '?'}`
      );
    } catch (e) {
      await message.channel.send(`Usage check error: ${e.message}`);
    }
    return true;
  }
  ...
  if (text === '/status') {
    ...
    const watchdogInfo = IS_WATCHDOG_MANAGED ? 'Yes' : 'No';
    const restartInfo = claudeRestartAttempts > 0 ? `${claudeRestartAttempts}/${CLAUDE_MAX_RESTART_ATTEMPTS}` : '0';
    const lastPtyAgo = Math.round((Date.now() - lastPtyOutputTime) / 1000);
    const lastResponseAgo = Math.round((Date.now() - lastClaudeResponseTime) / 1000);
    await message.channel.send(
`**Status**
Claude: ${claudeStatus}
PID: ${pid}
...
Last PTY output: ${lastPtyAgo}s ago
Last Claude response: ${lastResponseAgo}s ago
Auto-fallback: Disabled (use /models to switch)
Related processes: ${status.count}
Watchdog: ${watchdogInfo}
Auto-restart attempts: ${restartInfo}
...
Categories: bridge, claude, mcp, agent, system`
  ...
  if (text === '/restart') {
    ...
    claudeRestartAttempts = 0; // Reset backoff on manual restart
    if (claude) {
  ...
  if (text === '/reset') {
    ...
    claudeRestartAttempts = 0; // Reset backoff on manual reset
    if (claude) {
  ...
  // /fullrestart - Exit the entire bridge process (watchdog restarts it)
  if (text === '/fullrestart') {
    logger.logSystemCommand('/fullrestart');
    await message.channel.send(
      '**Full bridge restart initiated.**\n' +
      (IS_WATCHDOG_MANAGED
        ? 'Watchdog will auto-restart in 3-5 seconds...'
        : 'Bridge will exit. You need to restart manually with `npm start` or `npm run watchdog`.')
    );
    logger.loggers.system.info('FULL RESTART requested by user');

    // Kill Claude PTY first
    if (claude) {
      try { claude.kill(); } catch (e) {}
      claude = null;
    }
    killOrphanedClaude();

    // Give Discord time to send the message, then exit
    setTimeout(() => {
      logger.loggers.system.info('Exiting bridge process for full restart');
      process.exit(0); // Clean exit — watchdog will restart
    }, 2000);
    return true;
  }

  // /nuke - Kill EVERYTHING (all node processes related to teleclaude)
  if (text === '/nuke') {
    logger.logSystemCommand('/nuke');
    await message.channel.send(
      '**NUKE initiated.** Killing all Claude + bridge processes.\n' +
      'You will need to restart manually: `cd teleclaude-main && npm start`'
    );
    logger.loggers.system.info('NUKE requested by user');
    ...
    // Force exit after message sent
    setTimeout(() => {
      process.exit(1); // Non-zero exit — watchdog still restarts but logs the nuke
    }, 2000);
    return true;
  }
...
const TRADING_CHANNEL_CONFIG = [
  ...
  { key: 'macrostrategyV9', name: 'macrostrategy-v9', topic: 'MacroStrategy v9 genetic algorithm trading signals' },
  { key: 'iasmSignals', name: 'iasm-signals', topic: 'IASM (Intraday Adaptive Strategy Manager) trading signals' },
  { key: 'security', name: 'security', topic: 'Security alerts and audit reports' },
  { key: 'memoryHealth', name: 'memory-health', topic: 'Memory health and deduplication reports' }
];
...
async function setupTradingChannels(guild) {
  ...
  try {
    ...
    // Merge with existing config to preserve any manually-added channel IDs
    let existing = {};
    try {
      const raw = fs.readFileSync(tradingConfigPath, 'utf-8');
      existing = JSON.parse(raw);
    } catch (_) { /* no existing file, start fresh */ }
    const mergedChannels = Object.assign({}, existing.channels || {}, channelIds);
    fs.writeFileSync(tradingConfigPath, JSON.stringify({ guildId: guild.id, channels: mergedChannels }, null, 2));
  ...
  if (tradingChannels.general) {
    await tradingChannels.general.send(
      ...
      '• #macrostrategy-v9 - MacroStrategy V9 signals\n' +
      '• #iasm-signals - IASM signals\n' +
      '• #security - Security alerts and audit reports\n' +
      '• #memory-health - Memory health and deduplication reports\n\n' +
...
async function sendToTradingChannel(channelKey, message) {
  ...
  if (!channel) {
    // Channels that must NEVER fall back to DM — dedicated channel or silence
    const channelOnlyKeys = new Set([
      'memoryHealth', 'security', 'systemStatus', 'iasmSignals', 'macrostrategyV9',
      'pnl', 'tradeExecution', 'alerts', 'swingScanner', 'preMarket',
      'afterHours', 'overnight',
    ]);
    if (channelOnlyKeys.has(channelKey)) {
      console.log(`[${channelKey}] Channel not set up (channelOnly — NOT falling back to DM)`);
      return false;
    }
...
async function startDiscordMode(cfg) {
  ...
  setInterval(() => {
    try {
      ...
      if (stat.mtimeMs > lastMtime && stat.size > 0) {
        ...
        if (content) {
          // Track Claude response time, mark user message as answered
          lastClaudeResponseTime = Date.now();
          userMsgAnswered = true;
          ...
          if (responseChannel) {
            // Ensure currentChannel is set for subsequent sends
            if (!currentChannel) currentChannel = responseChannel;
      ...
      if (fs.existsSync(fileRequestPath)) {
        ...
        if (stat.size > 0) {
          try {
            ...
            if (request.filePath && (currentChannel || getResponseChannel())) {
              if (!currentChannel) currentChannel = getResponseChannel();
  ...
  client.on('messageCreate', async (message) => {
    ...
    // Store channel for responses (persist to survive restarts)
    currentChannel = message.channel;
    persistChannelId(message.channel.id);
    ...
    if (message.attachments.size > 0) {
      ...
      if (imageAttachments.length > 0) {
        ...
        // Verify files actually exist on disk (prevents ENOENT crashes downstream)
        const validPaths = localPaths.filter(p => p !== null && fs.existsSync(p));
  ...
  client.on('ready', async () => {
    ...
    if (guild) {
      ...
      if (!hasChannels) {
        ...
        // Save channel IDs to config for trading agents module (MERGE to preserve manually-added IDs)
        const channelIds = {};
        for (const [key, channel] of Object.entries(tradingChannels)) {
          if (channel) channelIds[key] = channel.id;
        }
        ...
        try {
          ...
          let existing = {};
          try {
            const raw = fs.readFileSync(savedConfigPath, 'utf-8');
            existing = JSON.parse(raw);
          } catch (_) { /* no existing file */ }
          const mergedChannels = Object.assign({}, existing.channels || {}, channelIds);
          fs.writeFileSync(savedConfigPath, JSON.stringify({ guildId: guild.id, channels: mergedChannels }, null, 2));
          console.log('Saved channel IDs to trading_agents config (merged)');
        } catch (err) {
          console.error('Failed to save channel config:', err.message);
        }
      }
  ...
  try {
    ...
    startHeartbeat();
    logger.loggers.system.info('Heartbeat started', { watchdogManaged: IS_WATCHDOG_MANAGED, isRestart: IS_RESTART });

    // Start usage monitor — checks every 5 min, alerts at 75%/90%/100%
    startUsageMonitor();

    // Watchdog handles crash recovery externally
