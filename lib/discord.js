...
function startHeartbeat() {
  ...
  heartbeatInterval = setInterval(() => {
    try { fs.writeFileSync(HEARTBEAT_FILE, Date.now().toString()); } catch (e) { /* ignore */ }
  }, 15_000); // Write heartbeat every 15s
  // Write immediately
  try { fs.writeFileSync(HEARTBEAT_FILE, Date.now().toString()); } catch (e) { /* ignore */ }
}
...
function startClaude() {
  ...
  claude.onExit((e) => {
    ...
    // Auto-restart Claude PTY with backoff
    if (claudeRestartAttempts < CLAUDE_MAX_RESTART_ATTEMPTS) {
      claudeRestartAttempts++;
      const delay = Math.min(CLAUDE_RESTART_DELAY_MS * Math.pow(1.5, claudeRestartAttempts - 1), 60_000);
      logger.loggers.system.info('Auto-restarting Claude PTY', {
        attempt: claudeRestartAttempts,
        maxAttempts: CLAUDE_MAX_RESTART_ATTEMPTS,
        delayMs: delay,
        previousUptime: `${Math.round(uptime / 1000)}s`
      });

      if (currentChannel) {
        currentChannel.send(`Claude crashed (uptime: ${Math.round(uptime / 1000)}s). Auto-restarting in ${Math.round(delay / 1000)}s... (attempt ${claudeRestartAttempts}/${CLAUDE_MAX_RESTART_ATTEMPTS})`).catch(() => {});
      }
      ...
      logger.loggers.system.error('Claude restart limit reached', { attempts: claudeRestartAttempts });
      if (currentChannel) {
        currentChannel.send(`Claude has crashed ${CLAUDE_MAX_RESTART_ATTEMPTS} times. Stopping auto-restart. Use /restart to manually restart.`).catch(() => {});
      }
    }
  });
